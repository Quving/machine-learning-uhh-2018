...
# Lines
def lines(data_y, data_x=None, title="", xlabel="", ylabel="", label="Lines", x_ticks=None, bins=None, savefig=True, figpath=default_dir, filename="lines_plot", format=default_format, grid=True, legend=True, clear=True, show=True, normalize=True, color=default_color, stackableplot=False):
    ...
    if x_ticks is not None:
        ...
        plt.plot(x_ticks_i, data_y, label=label)
    else:
        if data_x is None:
            plt.plot(data_y, label=label)
        else:
...
# Lines
def scatter(data_x, data_y, title="", xlabel="", ylabel="", label="Lines", savefig=True, figpath=default_dir, filename="lines_plot", format=default_format, grid=True, legend=True, clear=True, show=True, normalize=True, colors=default_color, markers=['o','^'], stackableplot=False):

    if clear:
        plt.gcf().clear()

    plt.scatter(data_x, data_y, label=label, c=colors)

    if not stackableplot:
        plotfinisher(title=title, xlabel=xlabel, ylabel=ylabel, savefig=savefig, figpath=figpath, filename=filename, format=format, grid=grid, legend=legend, show=show)

def heatmap(data_x, data_y, bins=(150,150), title="", xlabel="", ylabel="", label="Lines", savefig=True, figpath=default_dir, filename="lines_plot", format=default_format, grid=True, legend=True, clear=True, show=True, alpha=1, normalize=True, stackableplot=False):

    if clear:
        plt.gcf().clear()

    plt.hist2d(data_x, data_y, bins, cmap=plt.cm.jet, alpha=alpha, label=label)
...
def scatterheat(data_x, data_y, bins=None, title="", xlabel="", ylabel="", label="Lines", savefig=True, figpath=default_dir, filename="lines_plot", format=default_format, grid=True, legend=True, clear=True, show=True, alpha=1, normalize=True, stackableplot=False):

    if clear:
        plt.gcf().clear()

    data_stack = np.vstack([data_x, data_y])
    z = gaussian_kde(data_stack)(data_stack)

    plt.scatter(data_x, data_y, c=z, s=2.5, alpha=alpha)
